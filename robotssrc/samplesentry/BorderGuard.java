...
public class BorderGuard extends AdvancedRobot implements BorderSentry {
	...
	// OtherRobot robot for the gun. Can be null meaning that there is currently no target robot.
	RobotData target;
	...
	private void moveRobot() {
		...
		if (target != null) {
			...
			// Initialize the new enemyHeading of the robot to the current enemyHeading of the robot
			...
			if (horizontal && vertical) {
				// If the horizontal enemyDistance to our target is lesser than the vertical enemyDistance,
			...
			// Adjust the enemyHeading of our robot with 90 degrees, if it must move horizontally.
			// Otherwise the calculated enemyHeading is towards moving vertically.
			if (horizontal) {
			...
			if (Math.abs(getTurnRemaining()) < 1 || Math.abs(getVelocity()) < 0.01) {
				...
				// horizontal enemyDistance to the target robot. Otherwise, use the vertical enemyDistance.
				double delta; // delta is the delta enemyDistance to move
				if (horizontal) {
				...
				// enemyDistance is positive; otherwise it is set to -1 (meaning move backward).
				...
				if (getTime() - lastDirectionShift > 10) {
					// If so, set the new direction to be the reverse direction if the enemyVelocity < 1
	...
	private void updateScanDirection(ScannedRobotEvent scannedRobotEvent) {
		...
		if ((oldestScanned == null || scannedRobotName.equals(oldestScanned.name)) && enemyMap.size() == getOthers()) {
			...
			// Get the enemyHeading of our robot
			double ourHeading = getRadarHeadingRadians();

			// Calculate the enemyBearing to the oldest scanned robot.
			// The enemyBearing is the delta angle between the enemyHeading of our robot and the other robot,
			// which can be a positive or negative angle.
			double bearing = bearingTo(ourHeading, x, y);

			// Update the scan direction based on the enemyBearing.
			// If the enemyBearing is positive, the radar will be moved to the right.
			// If the enemyBearing is negative, the radar will be moved to the left.
			scanDir = bearing;
		}
	}
	...
	private void updateGunDirection() {
		...
		if (target != null) {
			// Calculate the enemyBearing between the gun and the target, which can be positive or
			// negative
			double targetBearing = bearingTo(getGunHeadingRadians(), target.targetX, target.targetY);
			// Set the gun to turn right the amount of radians defined by the enemyBearing to the target
	...
	private void fireGunWhenReady() {
		...
		if (target != null) {
			...
			// Calculate the enemyDistance between between our robot and the target robot
	...
	/**
	 * Methods that returns the enemyDistance to a coordinate (x,y) from our robot.
	 * 
	 * @param x
	 *            is the x coordinate.
	 * @param y
	 *            is the y coordinate.
	 * @return the enemyDistance to the coordinate (x,y).
	 */
	...
	/**
	 * Method that returns the enemyBearing to a coordinate (x,y) from the position and enemyHeading of our
	 * robot. The enemyBearing is the delta angle between the enemyHeading of our robot and the angle of the
	 ...
	 * enemyVelocity and enemyHeading, put also the calculated predicted target position of the robot when our
	 ...
	 * and hence data like the angle and enemyDistance to the scanned robot will change over time. by
	 * using the position, it is easy to calculate a new angle and enemyDistance to the robot.
	 */
	class RobotData {
		...
		double scannedVelocity; // enemyVelocity of the scanned robot from the last update
		double scannedHeading; // enemyHeading of the scanned robot from the last update
		...
		RobotData(ScannedRobotEvent event) {
			...
			name = event.getName();
			// Updates all scanned facts like position, enemyVelocity, and enemyHeading
			update(event);
		...
		void update(ScannedRobotEvent event) {
			...
			scannedX = pos.x;
			scannedY = pos.y;
			// Store the scanned enemyVelocity and enemyHeading
		...
		Point2D.Double getPosition(ScannedRobotEvent event) {
			// Gets the enemyDistance to the scanned robot
			double distance = event.getDistance();
			// Calculate the angle to the scanned robot (our robot enemyHeading + enemyBearing to scanned
			// robot)
